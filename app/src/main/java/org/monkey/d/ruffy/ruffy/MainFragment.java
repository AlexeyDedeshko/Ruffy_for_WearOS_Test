...
public class MainFragment extends Fragment implements View.OnClickListener {
    ...
    private final ServiceConnection mServiceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            ...
            // Переместим блок try-catch сюда
            try {
    ...
    private final Runnable upThread = new Runnable() {
        @Override
        public void run() {
            while (upRunning > 0) {
                if (upRunning == 1) {
    ...
    private final Runnable downThread = new Runnable() {
        @Override
        public void run() {
            while (downRunning > 0) {
                if (downRunning == 1) {
    ...
    private void rtSendKey(byte keyCode, boolean changed) {
        try {
            ...
            e.printStackTrace();
            appendLog("failed keySend: " + e.getMessage());
        }
    }
    ...
    private final IRTHandler.Stub handler = new IRTHandler.Stub() {
        @Override
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        bluetoothActivityResultLauncher = registerForActivityResult(
                ...
                result -> {
                    if (result.getResultCode() == Activity.RESULT_OK) {
                        // Bluetooth включен
                        appendLog("Bluetooth включен");
                    } else {
                        // Bluetooth не включен
                        appendLog("Bluetooth не включен");
                    }
                }
        );
        ...
        Button menu = displayLayout.findViewById(R.id.pumpMenu);
        ...
        Button check = displayLayout.findViewById(R.id.pumpCheck);
        ...
        Button up = displayLayout.findViewById(R.id.pumpUp);
        ...
        Button down = displayLayout.findViewById(R.id.pumpDown);
